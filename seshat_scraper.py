...
for polity in my_politys[250:]:
    source_url_seshat = 'https://seshat.info/w/index.php?title='
    source_url_seshatdb = 'http://seshatdatabank.info/browser/' 
    source_url = source_url_seshat + polity
    ...
    for goodie in main_text:
        ...
        if goodie.text not in helping_dic.values():
            key_for_unique = polity + '_' + str(inside_number_neat)
...
with open('all_polity_based_refs2.json', 'w') as fp:
    json.dump(big_dic, fp)

print(len(helping_dic))

with open('unique_polity_based_refs2.json', 'w') as fpf:
